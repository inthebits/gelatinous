...
class CmdUnslot(Command):
    ...
    def _show_comprehensive_view(self, caller):
        ...
        worn_by_location = {}
        if hasattr(caller, 'worn_items') and caller.worn_items:
            worn_by_location = caller.worn_items.copy()
        ...
        # Calculate spacing to align with boxes below
        # Location box is 15 chars wide, connector is 5 chars ("═════"), item box is 50 chars
        ...
        for location in valid_locations:
                ...
                for item in items_sorted:
                    ...
                    if getattr(item, 'is_plate_carrier', False):
                        ...
                        if affecting_plates:
                            ...
                            # Add each plate as sub-entry (marked for single-line connectors)
                            for plate in affecting_plates:
            ...
            if len(item_entries) == 1:
                ...
                # Format item text - no material type, just name and rating
                if entry['rating'] > 0:
                ...
                for idx, entry in enumerate(item_entries):
                    ...
                    # Check if next item is also a plate (for determining connector type)
                    next_is_plate = False
                    ...
                    if is_first:
                        # First item - show location box (always double-line frame)
                        output_lines.append(loc_box_top + "     " + "╔" + "═" * ITEM_BOX_WIDTH + "╗")
                        output_lines.append(loc_box_mid + connector + "║" + item_line + "║")
                        # No horizontal line after first item - just continue with vertical
                    elif is_last:
                        # Last item - close the stack with double-line (all items use ║)
                        output_lines.append(blank_space + "     " + "║" + item_line + "║")
                        output_lines.append(blank_space + "     " + "╚" + "═" * ITEM_BOX_WIDTH + "╝")
                    else:
                        # Middle item - all use double-line vertical ║
                        output_lines.append(blank_space + "     " + "║" + item_line + "║")
                        # Add horizontal separator only between non-plate items
                        if not is_plate and not next_is_plate:
                            # Both current and next are regular items - add separator
                            output_lines.append(blank_space + "     " + "╠" + "═" * ITEM_BOX_WIDTH + "╣")
                        elif not is_plate and next_is_plate:
                            # Current is regular, next is plate - no separator (plates group together)
                            pass
                        elif is_plate and not next_is_plate:
                            # Current is plate, next is regular - add separator to end plate group
                            output_lines.append(blank_space + "     " + "╠" + "═" * ITEM_BOX_WIDTH + "╣")
